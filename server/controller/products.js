...
class Product {
  ...
  async getProductByPrice(req, res) {
    ...
    console.log(price);
    if (!price) {
      return res.json({ error: "All filled must be required", price: price, text: "text" }
      );
    } else {
      try {
        let products = await productModel
          .find({ pPrice: { $lt: price } })
          // thêm trường cName trong bảng categories vào trường pCategory khi truy vấn dữ liệu
          .populate("pCategory", "cName")
          .sort({ pPrice: -1 })
        if (products) {
          return res.json({ Products: products });
